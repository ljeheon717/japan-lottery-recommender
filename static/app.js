...
function renderPurchaseList() {
  ...
  list.querySelectorAll(".purchase-del-btn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      btn.disabled = true;
      try {
        await fetch(`/api/purchases/${purchaseType}/${id}`, {
          method: "DELETE",
        });
...
document
  .getElementById("purchase-add-btn")
  .addEventListener("click", async () => {
    ...
    const numbers = document
      .getElementById("purchase-numbers")
      .value.split(",")
  ...
  btn.addEventListener("click", () => {
    ...
    if (genModes.has(mode)) {
      ...
      if (genModes.size === 1 && genModes.has("random") && mode !== "random") {
        ...
        document
          .querySelector('.mode-btn[data-mode="random"]')
...
async function loadMypickOptions() {
  ...
  try {
    ...
    selectedPurchaseIds = new Set(
      [...selectedPurchaseIds].filter((id) =>
        mypickOptions.some((p) => p.id === id),
      ),
    );
    renderMypickPanel();
  } catch (e) {
...
function renderPagination(page, total_pages, total) {
  ...
  if (end < total_pages)
    pages += `<button class="page-btn" data-page="${total_pages}">${total_pages}</button>`;
...
document
  ...
  .addEventListener("click", () => fetchResults(currentPage));
